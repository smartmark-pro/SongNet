...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, kv=None,
        ...
        if kv is None:
            x, self_attn = self.self_attn(
                query=x, key=x, value=x, key_padding_mask=self_padding_mask, attn_mask=self_attn_mask, need_weights=need_weights)
        else:
            x, self_attn = self.self_attn(
                query=x, key=kv, value=kv, key_padding_mask=self_padding_mask, attn_mask=self_attn_mask, need_weights=need_weights)
        ...
        if self.with_external:
            ...
            x, external_attn = self.external_attn(
                query=x, key=external_memories, value=external_memories, key_padding_mask=external_padding_mask, need_weights=need_weights)
            x = F.dropout(x, p=self.dropout, training=self.training)
    ...
    def work_incremental(self, x, self_padding_mask=None, self_attn_mask=None,
        ...
        if self.with_external:
            residual = x
            x, external_attn = self.external_attn(
                query=x, key=external_memories, value=external_memories, key_padding_mask=external_padding_mask)
            x = self.external_layer_norm(residual + x)
        else:
            external_attn = None
        residual = x
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False, incremental_state=None):
        ...
        if saved_state is not None:
            ...
            if 'prev_value' in saved_state:
                ...
                prev_value = prev_value.contiguous().view(
...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, input, offset=0):
        ...
        res = self.weights.index_select(0, positions).unsqueeze(
            1).expand(-1, bsz, -1).cuda(self.device).detach()
        return res
