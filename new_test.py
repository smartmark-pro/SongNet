...
def init_model(m_path, device, vocab):
    ...
    lm_model = BIGLM(device, lm_vocab, lm_args.embed_dim, lm_args.ff_embed_dim,
                     lm_args.num_heads, lm_args.dropout, lm_args.layers, 0.1)
...
def top_k(enc, src_padding_mask, inp_ys_tpl, inp_ys_seg, inp_ys_pos, s):
    ...
    res = []
    for l in range(inp_ys_tpl.size(0)):
        probs, pred = lm_model.work(
            enc, src_padding_mask, inp_y, inp_ys_tpl[0:l+1, :], inp_ys_seg[0:l+1, :], inp_ys_pos[0:l+1, :])
        next_tk = []
        for i in range(len(s)):
            ctk = lm_vocab.idx2token(inp_ys_tpl[l, i].item())
            if ctk != "<c1>":
...
def beam_decode(s, x, enc, src_padding_mask, inp_ys_tpl, inp_ys_seg, inp_ys_pos):
    ...
    for l in range(inp_ys_tpl.size(0)):
        ...
        last_scores = torch.FloatTensor(
            np.array(last_scores).reshape((num_live, 1))).to(gpu)
        next_y = []
...
# 定时, new_s2xy,
# 这块需要增加一个for循环处理部分段落, 预测也可能需要精准一些.
#
ds = []
