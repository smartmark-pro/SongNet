...
class BIGLM(nn.Module):
    def __init__(self, local_rank, vocab, embed_dim, ff_embed_dim, num_heads, dropout, layers, smoothing_factor, approx=None):
        ...
        for i in range(layers):
            self.layers.append(TransformerLayer(
                embed_dim, ff_embed_dim, num_heads, dropout, with_external=True))
        ...
        self.smoothing = LabelSmoothing(
            local_rank, self.vocab.size, self.vocab.padding_idx, smoothing_factor)
    ...
    def label_smotthing_loss(self, y_pred, y, y_mask, avg=True):
        ...
        loss = self.smoothing(y_pred.view(
            seq_len * bsz, -1), y.view(seq_len * bsz, -1))
        if avg:
    ...
    def encode(self, xs_tpl, xs_seg, xs_pos):
        ...
        x = self.tok_embed(xs_tpl) + self.tok_embed(xs_seg) + \
            self.tok_embed(xs_pos)
        x = self.emb_layer_norm(x)
    ...
    def forward(self, xs_tpl, xs_seg, xs_pos, ys_truth, ys_inp, ys_tpl, ys_seg, ys_pos, msk):
        ...
        padding_mask = torch.eq(
            ys_truth, self.vocab.padding_idx)  # 这是把真实值换成index么? 不是, 在batchify 已经替换了, 这里可能是bool值, 当作mask??
        if not padding_mask.any():
            padding_mask = None
        for layer in self.layers:
            x, _, _ = layer(x, self_padding_mask=padding_mask,
        ...
        # res, loss, acc, nll, ppl, ntokens, npairs
